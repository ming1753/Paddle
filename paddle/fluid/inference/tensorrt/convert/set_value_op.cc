...
#define GENERAL_GET_ATTR(input_name_, attr_name_)                                 \
  ...
  if (USE_INPUT(input_name_)) {                                                   \
    ...
    for (size_t i = 0; i < num_axes; ++i) {                                       \
      ...
      w_##attr_name_[i] = *dense_tensor->data<int>() < 0 ?                        \
              *dense_tensor->data<int>() + input_dims.d[axes[i]]                  \
              : *dense_tensor->data<int>();                                       \
    }                                                                             \
  } else {                                                                        \
    vec_##attr_name_ = PADDLE_GET_CONST(                                          \
            std::vector<int64_t>, op_desc.GetAttr(#attr_name_));                  \
    PADDLE_ENFORCE_EQ(                                                            \
  ...
  void operator()(const framework::proto::OpDesc& op,
    ...
    if (axes.empty()) {
      ...
      for (size_t i = 0; i < num_input_dims; ++i) {
        ...
        PADDLE_ENFORCE_NE(val_steps, 
                  0, 
                  "set_value_op step can not be zero");
        
        if (itensor_starts || itensor_ends || itensor_steps) {
          // TODO(ming1753): StartsTensorList or EndsTensorList or StepsTensorList may be less than 0 or greater than dims.d[i], not support now.
          if (itensor_starts == nullptr) {
            val_starts = std::max(0, val_starts);
            itensor_starts = Add1DConstantLayer<int>(val_starts, output_name + "_starts");
          }
          if (itensor_ends == nullptr) {
            val_ends = std::min(static_cast<int>(input_dims.d[i]), val_ends);
            itensor_ends = Add1DConstantLayer<int>(val_ends, output_name + "_ends");
          }
          if (itensor_steps == nullptr) {
            if (val_steps < 0) {
              ...
              val_starts += 1;
              val_steps = -val_steps;
              std::swap(val_starts, val_ends);
            }
            itensor_steps = Add1DConstantLayer<int>(val_steps, output_name + "_steps");
          }
          ...
          fill_layer->setInput(1, *Add1DConstantLayer<int>(val_starts, output_name + "_starts_scalar", true));
          ...
          if (val_steps > 0) {
            ...
            for (int j = val_ends + 1; j <= val_starts; j -= val_steps) {
              sequence.push_back(j);
            }
          }
          PADDLE_ENFORCE_GE(sequence.size(), 
                            0, 
                            "The length of the part to be set value must be greater than 0.");
          indices_length.push_back(Add1DConstantLayer<int>(static_cast<int>(sequence.size())));
          nvinfer1::Dims tmp_indices_dims;
          tmp_indices_dims.nbDims = static_cast<int>(num_input_dims + 1);
          for (int j = 0; j < tmp_indices_dims.nbDims; ++j) {
            if (j == static_cast<int>(i)) {
          ...
          auto tmp_indices = AddConstantLayer(sequence.data(), 
              tmp_indices_dims,"indices_"+std::to_string(i));
      ...
      for (size_t i = 0; i < num_input_dims; ++ i) {
        ...
        auto* concat_layer = TRT_ENGINE_ADD_LAYER(
            engine_, Concatenation, tmp_indices_length.data(), tmp_indices_length.size());
        ...
        fill_layer->setInput(2, *Add1DConstantLayer(std::vector<int>(num_input_dims + 1, 0)));
      ...
      auto* stack_layer = TRT_ENGINE_ADD_LAYER(
          engine_, Concatenation, indices_before_stack.data(), indices_before_stack.size());
      ...
      if (USE_INPUT(ValueTensor)) {
        ...
        auto* indices_count_tensor = GetEleTensorOfShape(indices_shape_tensor, 0);
        if (dtype == 2) {
          int value = PADDLE_GET_CONST(std::vector<paddle::experimental::Scalar>,
            op_desc.GetAttr("values"))[0].to<int>();
          auto fill_layer = TRT_ENGINE_ADD_LAYER(
            engine_, Fill, nvinfer1::Dims{}, nvinfer1::FillOperation::kLINSPACE);
          ...
          fill_layer->setInput(0, *indices_count_tensor);
          fill_layer->setInput(1, *Add1DConstantLayer(value, "", true));
          fill_layer->setInput(2, *Add1DConstantLayer(std::vector<float>(1, 0.)));
          updates = fill_layer->getOutput(0);
        }
      }
      ...
      if (dtype == 0) {
        ...
        ReplenishLayerAndOutput(cast_layer, "where_index", {output_name}, test_mode);
      } else {
